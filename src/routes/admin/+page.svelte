...
{#snippet toolbarButton(title: string, action: FormatAction, iconPath?: string, textIcon?: string)}
	<button
		type="button"
		{title}
	...
	>
		...
		{:else if textIcon}
			<span class="p-1.5 rounded hover:bg-zinc-800 text-zinc-400 hover:text-zinc-100 transition-colors cursor-pointer font-bold text-[11px] leading-none px-1">{textIcon}</span>
		{/if}
	</button>
{/snippet}
